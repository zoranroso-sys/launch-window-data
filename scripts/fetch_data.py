...
def fetch_igdb_upcoming(token, existing_titles):
    ...
    body   = f"""
        fields name, first_release_date, genres.name, platforms.name, hypes, summary;
...
def get_notable_releases():
    return [
        ...
        {"title": "Marvel's Wolverine",          "date": "2026-09-15", "genres": ["Action", "Adventure", "Superhero"],       "platforms": ["PS5"],
         ...
         "description": "Insomniac Games' Wolverine title in the Marvel's Spider-Man universe. Mature-rated action."},
        {"title": "007: First Light",            "date": "2026-10-13", "genres": ["Action", "Stealth", "FPS"],              "platforms": ["PS5", "Xbox", "PC"],
         ...
         "description": "IO Interactive's James Bond origin story. First-person action with Hitman DNA."},
        {"title": "Grand Theft Auto VI",         "date": "2026-11-19", "genres": ["Action", "Open World", "Crime"],         "platforms": ["PS5", "Xbox"],
         ...
         "description": "Rockstar Games' next open-world crime epic set in Leonida. Most anticipated game of the decade. Console exclusive at launch."},
        {"title": "Forza Horizon 6",             "date": "2026-10-01", "genres": ["Racing", "Open World"],                  "platforms": ["Xbox", "PC"],
         ...
         "description": "Next entry in the open-world racing franchise from Playground Games."},
        {"title": "Metal Gear Solid Collection Vol. 2","date":"2026-09-01","genres":["Action","Stealth","Remaster"],         "platforms": ["PS5", "Xbox", "PC"],
         ...
         "description": "Remastered collection of MGS2, MGS3, and Peace Walker. Konami's legacy revival."},
        {"title": "Ace Combat 8",                "date": "2026-10-01", "genres": ["Flight", "Action", "Simulation"],        "platforms": ["PS5", "Xbox", "PC"],
         ...
         "description": "Next-gen flight combat from Project Aces. Unreal Engine 5 aerial dogfighting."},
        {"title": "Control: Resonant",           "date": "2026-08-01", "genres": ["Action", "Shooter", "Supernatural"],     "platforms": ["PS5", "Xbox", "PC"],
         ...
         "description": "Remedy's sequel to Control. Supernatural action-shooter with reality-bending mechanics."},
        {"title": "Metro 2039",                  "date": "2026-10-01", "genres": ["FPS", "Horror", "Post-Apocalyptic"],     "platforms": ["PS5", "Xbox", "PC"],
         ...
         "description": "Next entry in the post-apocalyptic FPS series. Atmospheric survival horror in the Moscow Metro."},
        {"title": "Assassin's Creed Black Flag Remake","date":"2026-11-01","genres":["Action","Adventure","Open World"],     "platforms": ["PS5", "Xbox", "PC"],
         ...
         "description": "Full remake of the beloved pirate Assassin's Creed. Naval combat and Caribbean exploration rebuilt."},
        {"title": "Halo: Campaign Evolved",      "date": "2026-11-01", "genres": ["FPS", "Sci-Fi", "Remaster"],            "platforms": ["Xbox", "PC"],
         ...
         "description": "Remastered campaign of the original Halo: Combat Evolved for modern platforms."},
        ...
        {"title": "GTA VI (PC) [TBC]",           "date": "2027-02-01", "genres": ["Action", "Open World", "Crime"],         "platforms": ["PC"],
         ...
         "description": "Expected PC port of Grand Theft Auto VI. Based on historical GTA V pattern (1+ year after console)."},
        {"title": "Gears of War: E-Day [TBC]",   "date": "2027-06-01", "genres": ["Shooter", "Action", "Co-op"],           "platforms": ["Xbox", "PC"],
         ...
         "description": "Prequel to the Gears of War franchise. The Coalition's Unreal Engine 5 debut."},
    ]
...
def main():
    ...
    for nr in notable:
        if nr["title"].lower().replace(" [tbc]", "") not in existing_titles_lower:
            ...
            item = {
                ...
                "description": nr.get("description") or nr.get("notes", ""),
                "developer": nr.get("developer", ""),
    ...
    # Post-process events: ensure description, url, location fields exist
    for evt in all_events:
        if not evt.get("description"):
            evt["description"] = evt.get("notes", "")
        # Extract location from notes if not set
        if not evt.get("location"):
            n = evt.get("notes", "")
            for city in ["Las Vegas", "San Francisco", "Cologne", "Tokyo", "London", "Austin", "Boston",
                          "Seattle", "Los Angeles", "New York", "Shanghai", "Busan", "Kyoto", "Sao Paulo",
                          "Dubrovnik", "Malmo", "Brighton", "Paris", "Atlanta", "Jonkoping", "Munich"]:
                if city in n:
                    evt["location"] = city
                    break
    ...
    # Build flat items array for the calendar (combines releases + events)
    flat_items = []
    for r in all_upcoming:
        flat_items.append({
            ...
            "is_release": True,
            "category": "release",
            "tier": r.get("tier_override") or ("aaa" if r.get("hype_score", 0) >= 9000 else "aa" if r.get("hype_score", 0) >= 500 else "indie"),
    ...
    for evt in all_events:
        flat_items.append({
            ...
            "url": evt.get("url", ""),
            "tbc": "[TBC]" in evt.get("label", ""),
            "source": evt.get("source", "curated_event"),
        })
    flat_items.sort(key=lambda x: x.get("date") or x.get("approx_date") or "9999-12-31")
    ...
    # 8. Generate ICS calendar files
    print("📅 Generating ICS calendar files…")
    ics_path = os.path.join(os.path.dirname(OUTPUT_PATH), "games-industry.ics")
    generate_ics(all_upcoming, all_events, ics_path)

    # Events + AAA-only feed (no AA/indie releases)
    aaa_only = [r for r in all_upcoming if r.get("hype_score", 0) >= 9000 or r.get("tier_override") == "aaa"]
    ics_aaa_path = os.path.join(os.path.dirname(OUTPUT_PATH), "games-industry-events-aaa.ics")
    generate_ics(aaa_only, all_events, ics_aaa_path)
...
def generate_ics(upcoming_releases, events, output_path):
    ...
    for e in events:
        ...
        desc = ics_escape(
            f"{e.get('description') or e.get('notes', '')}\\n{duration_note}\\n"
    ...
    for r in upcoming_releases:
        ...
        platforms = ", ".join(r.get("platforms", [])[:5]) or ""
        game_desc = r.get("description", "")
        title = r["title"]
        desc = ics_escape(
